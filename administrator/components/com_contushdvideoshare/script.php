...
class Com_ContushdvideoshareInstallerScript
{
	...
	public function postflight($type, $parent)
	{
		...
		else
		{
			...
			$player_values = 'a:26:{s:6:"buffer";s:1:"3";s:5:"width";s:3:"630";s:6:"height";s:3:"500";'
...
`catid` varchar(100) CHARACTER SET utf8 NOT NULL
		...
		foreach ($modules as $module)
		{
			...
			if($name=="mod_HDVideoShareCategory")
			{
				$positionRight="";
			}
			...
			->set($db->quoteName('position') . ' = ' . $db->quote($positionRight))
		...
		<table class="adminlist">
			...
			<tbody>
				...
				<tr class="row1">
            <td class="key" colspan="2"><?php echo 'HD Video Share Category - ' . JText::_('Module'); ?></td>
            <td style="text-align: center;">
<?php
						// Check installed modules
						$query->clear()
								...
								->from($db->quoteName('#__extensions'))
								->where($db->quoteName('type') . ' = ' . $db->quote('module'))
								->where($db->quoteName('element') . ' = ' . $db->quote('mod_HDVideoShareCategory'));

						$db->setQuery($query);
						$categorymod_id = $db->loadResult();
